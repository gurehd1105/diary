...
<%
   ...
   // SELEct ~ As ~: 컬럼에 별칭주기
   //ex) SELECT emp_no As empNo  
   //	ArrayList로 변형전 emp_no 테이블의 이름을 empNo로 별칭을 지정함 

   PreparedStatement stmt = conn.prepareStatement(sql);
   ResultSet rs = stmt.executeQuery();
	//------------------- 모델레이어 -------------------------
	// 자원해제? 뷰에서 사용중 --> 해제전에 ResultSet -> ArrayList로 변형카피 --> 자원해체
	// DB - Emp테이블 행의 집합(ResultSet)을  -> JAVA - Emp클래스 타입(ArrayList)의 집합으로 
